...
.context {
  ...
  $bkpt: breakpoint($single-pixel-value, 'min-width', 'height', 'no-query');
  ...
  $bkpt: breakpoint($media-not-plus-fenced, 'min-width', 'max-width', 'media');
  $contexts: map-get($bkpt, 'context');
  @each $k, $v in $contexts {
    _#{$k}: inspect($v);
  }
  ...
  /* No Query */
  _test: "breakpoint(#{$no-query})";
  $bkpt: breakpoint($no-query, 'min-width', 'media', 'no-query');
  $contexts: map-get($bkpt, 'context');
  @each $k, $v in $contexts {
    _#{$k}: inspect($v);
  }
